...
void main() 
{
    ...
    vec2 mouseUV = uMousePosition * 0.5 + 0.5;

    float dist = distance(uv, mouseUV);

    float influence = smoothstep(0.05, 0.0, dist);

    float noiseValue = snoise(vec3(uv.x * 2.0, uv.y * 1.2, uTime * 0.1));
    vec3 noiseColor = mix(uLightColor,uDarkColor, noiseValue);


    vec3 glowColor = mix(noiseColor, vec3(0.8, 0.8,0.8), influence);
    ...
    vec4 finalColor = mix(vec4(glowColor, 1.0), textureColor, 0.3);
